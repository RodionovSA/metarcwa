...
changes it into the vacuum background's *mode-amplitude* basis via the gap
matrix ``Phi0 = [[W0, W0], [V0, -V0]]`` and converts the resulting transfer
matrix ``M`` to the repo's S-matrix convention::
...
slice is a true vacuum-embedded S-matrix, so the recombination is exact, not
an approximation -- only the per-slice exponent shrinks.
...
def transfer_to_smatrix(T: Block2x2, background: Background) -> Block2x2:
    ...
    ones, never materializing a monolithic ``Phi0^-1``.

    Both faces sit in the same vacuum background, so the field vector at
    each face is ``psi = Phi0 @ (c+, c-)`` with the vacuum gap matrix
    ``Phi0 = [[W0, W0], [V0, -V0]]`` (``W0`` is always the identity, see
    ``LayerSolver._prepare_vacuum``). Substituting into ``psi(d) = T
    psi(0)`` with ``T`` partitioned to match the ``(s, u)`` structure of the
    first-order system (``T.a/.b/.c/.d`` = ``T11/T12/T21/T22``) and grouping
    outgoing (``c_L-``, ``c_R+``) vs. incoming (``c_L+``, ``c_R-``) amplitudes
    gives exactly the boundary-matching linear system ``left @ (c_R+, c_L-)
    = right @ (c_L+, c_R-)`` that :func:`S_boundary` solves — just built from
    ``T`` instead of plain continuity. One ``Block2x2.solve`` (never
    ``.inv()``, and only one solve, not two) gives the whole S-matrix.

    Why this matters over the naive ``Phi0^-1 T Phi0`` route: at reduced
    precision (``dtype=torch.float32``), forming ``Phi0^-1`` explicitly is
    ill-conditioned whenever the background has evanescent harmonics with
    widely differing magnitudes (common at high truncation / short
    ...
    Sanity check: for a vacuum layer, this reduces exactly to
    :func:`~metarcwa.solver.smatrix.S_prop`.
    """
...
class TransferOperator:
    ...
    def smatrix(self, background: Background, left: bool = True) -> Block2x2:
        """Assemble the S-matrix: slice, exponentiate each slice, convert to
        an S-matrix, recombine via :func:`star_power`.
